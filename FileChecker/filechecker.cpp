...
FileChecker::FileChecker(QWidget *parent) : QDialog(parent), ui(new Ui::FileChecker)
{
    ui->setupUi(this);
    ...
    for(int i = 0; i < files.size(); ++i)
    {
        ...
        if(file.open(QIODevice::ReadOnly))
        {
            ...
            if(QFile::exists(QDir::toNativeSeparators(path2 + files[i])))
            {
                ...
                {
                    diffApp->close();
                    bool result = diffApp->completeInit(path2 + files[i], directory.absoluteFilePath(files[i]));
...
void FileChecker::createDiff()
{
    if(!diffApp)
    {
        diffApp = new KDiff3App(ui->vSplitter, "KDiff3App", extensions);
        QList<int> sizes;
        sizes.clear();
        sizes.append(80);
...
void FileChecker::acceptFile()
{
    ...
    if(item)
    {
        ...
        {
//            QPixmap pix(":/images/ok.svg");
//            QPixmap resPix = pix.scaled(17,17, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
            QPixmap pix = QIcon(":/images/ok.svg").pixmap(17, 17);
            QLabel *lbl = new QLabel;
            lbl->setPixmap(pix);
            lbl->setAlignment(Qt::AlignCenter);
...
void FileChecker::deleteFile()
{
    ...
    if(item)
    {
        ...
        if(QFile::exists(path1))
            ...
            {
//                QPixmap pix(":/images/cancel.svg");
//                QPixmap pix(QIcon(":/images/cancel.svg").pixmap(QSize()));
//                QPixmap resPix = pix.scaled(17,17, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
                QPixmap pix = QIcon(":/images/cancel.svg").pixmap(17, 17);
                QLabel *lbl = new QLabel;
                lbl->setPixmap(pix);
                lbl->setAlignment(Qt::AlignCenter);
...
void FileChecker::filesTableCurrentCellChanged(int row, int col, int pRow, int pCol)
{
    ...
    if(item)
    {
        ...
        {
            okBtn = new QToolButton();
            okBtn->setIcon(QIcon(":/images/ok.svg"));
            okBtn->setToolTip(tr("Accept received file and move it to selected search path"));
            connect(okBtn, SIGNAL(clicked()), this, SLOT(acceptFile()));

            noBtn = new QToolButton();
            noBtn->setIcon(QIcon(":/images/cancel.svg"));
            noBtn->setToolTip(tr("Delete received file"));
            connect(noBtn, SIGNAL(clicked()), this, SLOT(deleteFile()));
