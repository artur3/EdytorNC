...
void EdytorNc::copySettingsToFile()
{


    QSettings dst(QSettings::IniFormat, QSettings::UserScope, "EdytorNC", "EdytorNC");

    if(!dst.contains("FileInUse"))
    {
        qDebug() << "File does not exist";

        QSettings src("EdytorNC", "EdytorNC");
...
void EdytorNc::closeEvent(QCloseEvent *event)
{
    ...
    foreach(const QMdiSubWindow *window, mdiArea->subWindowList(QMdiArea::ActivationHistoryOrder))
    {
...
#if defined(Q_OS_WIN)
        QPrinter printer(QPrinter::ScreenResolution);  //  Fix (?) for too small fonts when higher resolution display is used
#else
...
{
    QPalette palette;
    bool hasMdiChildNotReadOnly = ((activeMdiChild() != (void *) NULL) && !activeMdiChild()->isReadOnly());
    bool found = false;
...
void EdytorNc::about()
{
    QMessageBox::about(this, tr("About EdytorNC"),
                       ...
                       tr("<P>Spanish translation, fixes and other help thanks to Otto Huamani") +
                       tr("<br />Catalan translation and deb package thanks to Jordi Sayol i Salomó") +
                       tr("<br />German translation and other fixes thanks to Michael Numberger") +
                       tr("<br />Czech translation thanks to Pavel Fric") +
                       tr("<br />Finnish translation thanks to Janne Mäntyharju") +
                       tr("<br />Dutch translation thanks to Henk van Andel") +
                       tr("<br />OS X patch and other updates thanks to Janne Mäntyharju") +
                       tr("<P>New EdytorNC icon thanks to Jakub Gajewski") +
                       tr("<P><a href=\"http://sourceforge.net/projects/edytornc/\">http://sourceforge.net/projects/edytornc</a>") +
                       tr("<P><a href=\"https://plus.google.com/communities/117267107585407507443/\">EdytorNC Google Plus Community</a>") +
                       tr("<P>") +
...
void EdytorNc::updateMenus()
{
    ...
    MdiChild *childWindow = activeMdiChild();
    bool hasMdiChild = (childWindow > (void *) NULL);

    if(hasMdiChild)
    {
        if(!childWindow->hasFocus())
            return;
...
void EdytorNc::createActions()
{
    ...
    printPreviewAct = new QAction(QIcon(":/images/document-print-preview.svg"), tr("Pr&int preview"), this);
    ...
    sessionMgrAct->setToolTip(tr("Session manager"));
    ...
    redoAct = new QAction(QIcon(":/images/edit-redo.svg"), tr("&Redo"), this);
    ...
    selAllAct = new QAction(QIcon(":/images/edit-select-all.svg"), tr("&Select all"), this);
    ...
    calcAct = new QAction(QIcon(":/images/calc.svg"), tr("Calculator"), this);
    //calcAct->setShortcut(tr("F9"));
    calcAct->setToolTip(tr("Run calculator"));
    connect(calcAct, SIGNAL(triggered()), this, SLOT(doCalc()));

    showSerialToolBarAct = new QAction(QIcon(":/images/serial.svg"), tr("Serial port send/receive"), this);
    ...
    closeAct = new QAction(QIcon(":/images/file-close.svg"), tr("Cl&ose \"%1\"").arg(""), this);
    //closeAct->setShortcut(QKeySequence::Close);
    closeAct->setToolTip(tr("Close the active window"));
    connect(closeAct, SIGNAL(triggered()), this, SLOT(closeCurrentWindow()));

    closeAllAct = new QAction(QIcon(":/images/window-close.svg"), tr("Close &All"), this);
    ...
    nextAct = new QAction(QIcon(":/images/go-next.svg"), tr("Ne&xt"), this);
    nextAct->setShortcut(QKeySequence::Forward);
    nextAct->setToolTip(tr("Move the focus to the next window"));
    connect(nextAct, SIGNAL(triggered()), mdiArea, SLOT(activateNextSubWindow()));

    previousAct = new QAction(QIcon(":/images/go-previous.svg"), tr("Pre&vious"), this);
...
void EdytorNc::createFindToolBar()
{
    ...
    if(!findToolBar)
    {
        ...
        findCloseAct = new QAction(QIcon(":/images/close_small.svg"), tr("Close find toolbar"), this);
...
void EdytorNc::createSerialToolBar()
{
    if(serialToolBar == (void *) NULL)
    {
        ...
        serialToolBar->addAction(FTPAppAct);
...
void EdytorNc::projectSave()
{
    ...
    if(currentProjectName.isEmpty() || currentProject == (void *) NULL)
        return;
...
void EdytorNc::updateOpenFileList()
{
    ...
    for(int i = 0; i < windows.size(); ++i)
    {
        ...
        newItem = new QTableWidgetItem(QIcon(":/images/file-close.svg"), "", QTableWidgetItem::UserType);
...
void EdytorNc::fileTreeViewChangeRootDir()
{
    ...
    if((fileTreeView == (void *) NULL) || (dirModel == (void *) NULL))
        return;

    if(currentPathCheckBox->isChecked() && (activeMdiChild() > (void *) NULL))
    {
...
void EdytorNc::savePrinterSettings(QPrinter *printer)
{
    ...
    settings.setValue("PageUnits", layout.units());
    settings.setValue("MarginL", layout.margins().left());
    settings.setValue("MarginT", layout.margins().top());
    settings.setValue("MarginR", layout.margins().right());
    settings.setValue("MarginB", layout.margins().bottom());
...
void EdytorNc::loadPrinterSettings(QPrinter *printer)
{
    ...
    layout.setUnits((QPageLayout::Unit)settings.value("MarginUnits", layout.units()).toInt());
    layout.setLeftMargin(settings.value("MarginL", layout.margins().left()).toReal());
    layout.setTopMargin(settings.value("MarginT", layout.margins().top()).toReal());
    layout.setRightMargin(settings.value("MarginR", layout.margins().right()).toReal());
    layout.setBottomMargin(settings.value("MarginB", layout.margins().bottom()).toReal());
...
#ifdef Q_OS_LINUX
    fileName = path + "SerialPorFileServer";
#endif

#ifdef Q_OS_WIN32
    fileName = "SerialPortFileServer.exe";
#endif
...
void EdytorNc::clipboardChanged()
{
    ...
    if(notFound)
    {
        ...
        item = new QStandardItem(QIcon(":/images/edit-paste.svg"), "");
...
//    Q_UNUSED(point);
    ...
    while(it != list.end())
    {
        ...
        if(!text.isEmpty())
        {
            ...
            QToolTip::showText(clipboardTreeView->mapToGlobal(point), tr("Copied"));
        };
...
void EdytorNc::clipboardLoad()
{
    ...
    for(int i = 0; i < max; ++i)
    {
        ...
        item = new QStandardItem(QIcon(":/images/edit-paste.svg"), itemText);
        item->setEditable(true);
        font = item->font();
        font.setBold(true);
        font.setPointSize(font.pointSize() + 1);
